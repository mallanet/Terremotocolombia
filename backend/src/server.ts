...
import { developmentPinMissing } from "@/tenant/hostname";
...
if (isNodeEntrypoint()) {
  if (developmentPinMissing(env.NODE_ENV, env.PINNED_DEPLOYMENT_HOSTNAME)) {
    console.error(
      "❌ PINNED_DEPLOYMENT_HOSTNAME is required in development (U9). Example: localhost",
    );
    process.exit(1);
  }
  app.listen(env.PORT, () => {
