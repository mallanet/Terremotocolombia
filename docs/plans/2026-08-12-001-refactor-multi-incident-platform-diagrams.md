...
**Phase 0 note (2026-08-21):** current Colombia `origin/main` is `83b7c16`. Live modules now also include reconstruction campaign and official deceased lists. Draw them with the other domain modules when a later diagram refresh happens. The topology, trust, cache, and autonomy diagrams remain the target architecture.
