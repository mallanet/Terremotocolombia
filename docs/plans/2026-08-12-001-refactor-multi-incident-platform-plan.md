...
execution_ledger: docs/plans/2026-08-22-001-multi-incident-platform-execution-ledger.md
...
**Execution ledger:** [2026-08-22 execution ledger](./2026-08-22-001-multi-incident-platform-execution-ledger.md)

**Phase 0 reconciliation (2026-08-22):** this plan's original review SHA is `89089da`. Current immutable `origin/main` is `83b7c16` (67 commits later, **10 first-parent**: official deceased lists, reconstruction-campaign tables/routes, Stripe Checkout donations, volunteer ficha, family-query performance, and `/api/deceased` on the JSON edge-cache allowlist). Do not bootstrap from `89089da` or from a dirty feature branch. U6/U19 record `83b7c16` unless a later fetch moves `origin/main` before clone. U7 classification and U13 domain extraction must include campaign reconstruction and official deceased. The donations DDD module is an external Stripe Checkout adapter plus Payment Links; it is not Mallanet money movement. Cache inventory at `83b7c16`: 24 `cached()` sites and 35 `invalidate()` call sites. The column-drift gate at `83b7c16` imported only `schema.ts` and missed campaign tables; U0 includes `schema-campaign.ts`. `workers_dev` is unset (Cloudflare default true). No platform repository existed at plan write time. U6 bootstrapped
`Emuthmartinez/platform` from `83b7c16` as an interim home until an org
owner can place `mallanet/platform`.
