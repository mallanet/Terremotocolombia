...
- `packages/contracts/`: shared Zod envelopes (`file:` dependency). Backend,
  ...
  raising `turbopack.root` and nesting the standalone output. The three
  list shapes and the additive-only rule are in
  `packages/contracts/README.md`. A contracts-only merge can upload
...
`api-staging.mallanet.org`. Production keeps the Colombia API. The staging
workflow passes `API_OVERRIDE` to its domain and job checks, and verifies the
frontend/admin SHA independently from the platform API release SHA. The
platform API allows only the Colombia staging web origin for this bridge. This
does not change Colombia DNS or production traffic.
